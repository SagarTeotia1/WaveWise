...
Shader "Skybox/Panoramic(LDR)" {
    Properties {
        _Tint ("Tint Color", Color) = (.5, .5, .5, .5)
        [Gamma] _Exposure ("Exposure", Range(0, 8)) = 1.0
        _Rotation ("Rotation", Range(0, 360)) = 0
        [NoScaleOffset] _MainTex ("Spherical  (LDR)", 2D) = "grey" {}

        [Header(Color Adjustment)]
        [Toggle(_USE_HUE_SHIFT)] _UseHueShift("Shift hue?", Float) = 0
        _HueShiftAngle("Hue shift Angle", Range(-1,1)) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            #pragma shader_feature_local_fragment _USE_HUE_SHIFT
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                // Shift hue
                #if defined(_USE_HUE_SHIFT)
                    c.rgb = ApplyHue(c.rgb, _HueShiftAngle);
                #endif
